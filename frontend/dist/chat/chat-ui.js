...
export class ChatUI {
    ...
    render() {
      ...
      <div class="flex h-[calc(100vh-8rem)] bg-gray-800/70 backdrop-blur-xl rounded-2xl overflow-hidden border border-gray-700/50 shadow-2xl mt-20">
            ...
            <header class="bg-gray-900/50 border-b border-gray-700/50 px-6 py-4 flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span id="chat-status" class="text-emerald-400 text-sm">● Online</span>
                </div>
              </div>
        ...
        <!-- Connection Status -->
        <div id="connection-status" class="hidden fixed bottom-4 right-4 px-4 py-2 rounded-lg bg-red-500/90 text-white text-sm font-semibold shadow-lg">
          ⚠️ Disconnected
        </div>
    ...
    attachEventListeners() {
        ...
        document.getElementById('chat-invite-game-btn')?.addEventListener('click', () => {
            // Get the current friend's user ID from the chat room data
            // This will be set when joining a room
     ...
     * Render friends list
     */
    async renderFriendsList(friends, currentUserId) {
    ...
    async filterFriendsList(query) {
        const container = document.getElementById('chat-friends-list');
        if (!container)
            return;
        let friends = this.allFriends;
        if (query) {
            friends = friends.filter(friend => friend.username.toLowerCase().includes(query));
        }
        if (friends.length === 0) {
        ...
        try {
            ...
            if (blockedUsersResponse.ok) {
                ...
                blockedUserIds = blockedUsers.map((block) => block.blocked.id);
            }
        }
        catch (error) {
        ...
        container.innerHTML = friends
            ...
            return `
          <div class="flex items-center gap-3 p-3 mb-2 bg-gray-800/40 hover:bg-gray-700/50 rounded-lg cursor-pointer transition-all group border border-transparent hover:border-emerald-500/30" data-friend-id="${friend.id}">
            <div class="relative">
              ...
              <span class="absolute bottom-0 right-0 text-xs">${statusDot}</span>
            </div>
            ...
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button class="p-1.5 hover:bg-gray-600/50 rounded transition-colors text-lg" data-action="view-profile" data-user-id="${friend.id}" title="View Profile">👤</button>
              <button class="p-1.5 hover:bg-emerald-500/20 rounded transition-colors text-lg" data-action="message-friend" data-user-id="${friend.id}" title="Message">💬</button>
              <button class="p-1.5 hover:bg-emerald-500/20 rounded transition-colors text-lg" data-action="invite-friend" data-user-id="${friend.id}" title="Invite to Game">🎮</button>
              <button class="p-1.5 rounded transition-colors ${isBlocked ? 'hover:bg-emerald-500/20 text-emerald-400' : 'hover:bg-red-500/20 text-red-400'}" data-action="${isBlocked ? 'unblock' : 'block'}-friend" data-user-id="${friend.id}" title="${isBlocked ? 'Unblock' : 'Block'}">${isBlocked ? '✓' : '🚫'}</button>
            </div>
          </div>
        `;
        }).join('');
        ...
        container.querySelectorAll('[data-action="message-friend"]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const userId = parseInt(e.target.dataset.userId || '0');
        ...
        container.querySelectorAll('[data-action="invite-friend"]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const userId = parseInt(e.target.dataset.userId || '0');
        ...
        container.querySelectorAll('[data-action="block-friend"]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const userId = parseInt(e.target.dataset.userId || '0');
        ...
        container.querySelectorAll('[data-action="unblock-friend"]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const userId = parseInt(e.target.dataset.userId || '0');
     ...
     */
    showFriendRequestsModal(requests) {
    ...
    `;
        ...
        modal.querySelectorAll('[data-action="accept-request"]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const requestId = parseInt(e.target.dataset.requestId || '0');
    ...
    filterRoomsList(query) {
        ...
        if (query) {
            publicRooms = publicRooms.filter(room => room.name.toLowerCase().includes(query));
        }
      ...
      `;
        ...
        container.querySelectorAll('[data-room-id]').forEach(item => {
            item.addEventListener('click', (e) => {
                const roomId = parseInt(e.currentTarget.dataset.roomId || '0');
    ...
    renderMessages(messages, currentUserId) {
        ...
        if (!container)
            return;
        container.innerHTML = messages.map(msg => this.createMessageHTML(msg, currentUserId)).join('');
        this.scrollToBottom();
    }
    /**
     * Add single message
    ...
    showChatArea(show) {
        ...
        if (activeChat)
            activeChat.classList.toggle('hidden', !show);
    }
    /**
     * Set chat title/header with avatar
     */
    setChatTitle(title, targetUserId, roomType, roomOwnerId, currentUserId, avatarUrl, isBlocked) {
        ...
        // Get action buttons and status indicator
        ...
        // First, hide all buttons and status by default
        if (gameBtn)
        ...
        if (roomType === 'private' && targetUserId) {
            // Private chat - show all user action buttons and status
            if (gameBtn) {
    ...
    async showUserProfile(userId) {
        try {
            // Fetch profile data
            const profile = await fetch(`/api/auth/user/${userId}`, {
                headers: {
            ...
            const { blockedUsers } = await blockedUsersResponse.json();
            const isBlocked = blockedUsers.some((block) => block.blocked.id === userId);
        ...
        <div class="bg-gray-800 rounded-2xl p-6 max-w-md w-full mx-4 border border-gray-700 shadow-2xl">
          ...
          <div class="space-y-3 bg-gray-900/50 rounded-lg p-4 border border-gray-700/50">
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="text-white font-medium">#${profile.id}</span>
            </div>
          </div>
     ...
     */
    async showPasswordPrompt(roomName) {
      ...
      `;
            document.body.appendChild(modal);
            const passwordInput = document.getElementById('room-password-prompt-input');
            // Focus on input
     ...
     */
    showGameInviteNotification(invite) {
