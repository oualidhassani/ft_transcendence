...
export function cleanupTournamentMatch() {
    ...
    if (gameUpdateListener)
        removeMessageListener(gameUpdateListener);
    if (gameFinishListener)
        removeMessageListener(gameFinishListener);
    if (inputInterval)
...
function setupTournamentInput(gId, pId) {
...
export function createTournamentListener(userId, tournamentId, navigateCallback) {
    const views = {
    ...
    const els = {
        ...
        readyBtn: getElement("game-ready-btn"),
    ...
    const switchView = (view) => {
        if (views.lobby)
            views.lobby.style.display = view === 'lobby' ? 'block' : 'none';
        if (views.bracket)
            views.bracket.style.display = view === 'bracket' ? 'flex' : 'none';
        if (views.game)
            views.game.style.display = view === 'game' ? 'block' : 'none';
    };
    const resolveUser = async (pid) => {
    ...
    const createMatchHTML = (p1, p2, label) => `
  ...
  `;
    ...
    const initGameConfig = (msg) => {
        ...
        gameUpdateListener = (updateMsg) => {
            if (updateMsg.type === "game_update" && ctx) {
                ...
                if (scoreEl)
                    scoreEl.innerText = `${gameState.paddles.left.score} - ${gameState.paddles.right.score}`;
        ...
        gameFinishListener = (finishMsg) => {
            if (finishMsg.type === "game_finish") {
                console.log("🏁 Match Finished");
                cleanupTournamentMatch();
                if (els.gameLabel)
                    els.gameLabel.innerText = "Match Ended";
            }
        };
    ...
    const runCountdown = (seconds, onComplete) => {
        let count = seconds;
        if (els.timer)
            els.timer.innerText = String(count);
    ...
    return (msg) => {
        if (msg.type === "game_config") {
            ...
            if (els.readyBtn) {
                els.readyBtn.onclick = () => {
                    ...
                    initGameConfig(msg);
                    setTimeout(() => { if (els.readyOverlay)
                        els.readyOverlay.style.display = 'none'; }, 500);
                };
            }
            return;
        }
        switch (msg.type) {
            case "tournament_semi-finals":
                switchView('bracket');
                if (els.bracketTitle)
                    els.bracketTitle.innerText = "SEMI-FINALS";
            ...
            case "tournament_final":
                switchView('bracket');
                if (els.bracketTitle)
                    els.bracketTitle.innerText = "GRAND FINAL";
                resolveAndRenderBracket(msg.payload.final.players[0], msg.payload.final.players[1]).then(({ html, amIPlaying, users }) => {
                    if (els.bracketContent)
                        els.bracketContent.innerHTML = html;
                    runCountdown(5, () => {
                        if (amIPlaying) {
...
export function setupTournamentGameListeners(tournamentListener) {
